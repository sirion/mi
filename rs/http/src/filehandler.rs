...
impl<'a> FileHandler<'a> {
	...
	fn list_dir(&self, mut res: super::Response, path: PathBuf) -> Result<(), std::io::Error> {
		let entries = match std::fs::read_dir(&path) {
			Ok(e) => e,
			Err(e) => {
				log_err(format!("Internal Sever Error - Cannot list directory {}: {}", path.to_string_lossy(), e));
		...
		for entry in entries {
			let file = match entry {
				Ok(f) => f,
				Err(e) => {
					log_err(format!("Error reading directory index: {}", e));
					continue;
				}
			};
			...
			let name = match file_name.to_str() {
				Some(s) => s,
				None => {
					log_err(format!("Error reading directory index: {}", file.path().to_string_lossy()));
					continue;
				}
			};
	...
	fn serve_file(&self, mut res: super::Response, path: PathBuf) -> Result<(), std::io::Error> {
		match std::fs::read(&path) {
			...
			Err(e) => {
				log_err(format!("Internal Sever Error: {}", e));
				return self.serve_error(res, 500, "Internal Server Error");
			}
		}
	}
	...
	fn serve(
		...
		if path.exists() {
			...
			if is_dir && has_slash {
				...
				log_err(format!("Internal Sever Error - path is neither file nor directory: {}", path.to_string_lossy()));
