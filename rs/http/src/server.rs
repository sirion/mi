...
pub struct Server {
	...
	/// Writer to which to log debug output. Defaults to ignored
	pub log_debug: Arc<Mutex<dyn Write + Send>>,
	running: bool,
	handlers: Vec<Arc<dyn super::RequestHandler + Sync>>,
}
...
impl Server {
	...
	pub fn new() -> Server {
		Server {
			...
			log_debug: Arc::new(Mutex::new(std::io::sink())),
			running: true,
			handlers: Vec::new(),
		}
	}
	...
	pub fn handle(
		&mut self,
		matcher_fn: Arc<Mutex<dyn FnMut(&super::Request) -> bool + Sync + Send>>,
		handler_fn: Arc<Mutex<dyn FnMut(&super::Request, super::Response) + Sync + Send>>,
	) {
		self.handler(Arc::new(super::Handler::new(matcher_fn, handler_fn)));
	}

	/// Adds a [super::RequestHandler] to the server.
	pub fn handler(&mut self, handler: Arc<dyn super::RequestHandler + Sync>) {
	...
	pub fn listen(&mut self, port: u16) -> Result<(), Box<dyn std::error::Error>> {
		...
		for st in listener.incoming() {
			let stream = match st {
				Ok(s) => s,
				Err(e) => {
					log(&self.log_errors, format!("Incoming connection error: {}", e));
					continue;
				}
			};
			...
			if !self.running {
				log(&self.log_debug, "Stopping Server");
				break;
			}
		}
	...
	fn handle_connection(&mut self, pool: &ThreadPool, stream: TcpStream) {
		match stream.set_read_timeout(self.read_timeout) {
			Ok(()) => (),
			Err(e) => log(&self.log_errors,	format!("Error setting read timeout: {}", e)),
		};
		...
		let req = match r {
			Ok(r) => r,
			Err(e) => {
				log(&self.log_errors, format!("x: Invalid Request: {}", e.to_string()));
				return;
			}
		};
		...
		let mut matched_handler: Option<Arc<dyn super::RequestHandler + Sync>> = None;
		...
		let response_stream = match req.clone_stream() {
			Ok(s) => s,
			Err(e) => {
				log(&self.log_access, format!("Could not clone response stream: {}", e));
				return;
			}
		};
