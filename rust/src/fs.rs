...
pub fn list_dir(dir: &PathBuf) -> Vec<PathBuf> {
	...
	if !dir.is_dir() {
		errorln(format!("Not a directory: {}", dir.to_string_lossy()));
		return files;
	};
...
/// Very simple way of hashing a file. Not safe, can have collisions.
pub fn file_hash_quick(p: &PathBuf) -> u64 {
	crate::bin::hash_quick(std::fs::read(p).unwrap())
}
